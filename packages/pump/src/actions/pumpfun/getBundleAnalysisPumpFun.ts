...
const getBundleAnalysisPumpFunAction: Action = {
  ...
  description: "Analyze trading bundles and patterns for a token on Pump.fun",
  examples: [
    [
      {
        input: {
        ...
        output: {
          ...
          message: "Found 1 bundle(s) with total trades: 150",
        },
        explanation: "Get bundle analysis for a token on Pump.fun",
      },
    ],
  ],
  ...
  handler: async (
    ...
    try {
      ...
      return {
        status: "success",
        message: `Found ${analysis.bundles.length} bundle(s) with total trades: ${analysis.totalTrades}. As I mentioned, you can see the bundle details in the UI. Do not re-iterate this message.`,
        data: analysis,
      };
    } catch (error: any) {
      return {
