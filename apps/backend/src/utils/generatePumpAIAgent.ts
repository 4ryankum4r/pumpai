import { PumpAIAgent, Cluster } from "@repo/pumpai-agent";
...
interface GeneratePumpAIAgentParams {
  address: string;
  cluster: Cluster;
}

export function generatePumpAIAgent({
  address,
  cluster,
}: GeneratePumpAIAgentParams): PumpAIAgent {
  const rpcUrl =
